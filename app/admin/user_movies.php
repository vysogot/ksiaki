...
<div class="wrapper">
  <h2><?= t('user_movies') ?></h2>
  <p><?= link_to(t('add_new'), '/admin/user_movies/new.php') ?></p>
  <table>
    <?php foreach($data as $user_movie) { ?>
      <tr>
        ...
        <td><?= link_to(t('show'), "/admin/user_movies/show.php?id=$user_movie->id") ?></td>
        <td><?= link_to(t('edit'), "/admin/user_movies/edit.php?id=$user_movie->id") ?></td>
        <td><?= link_to(t('delete'), "/admin/user_movies/delete.php?id=$user_movie->id", ['class' => 'confirmation']) ?></td>
      </tr>
    <?php } ?>
  </table>
</div>
