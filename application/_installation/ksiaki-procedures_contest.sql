...
DELIMITER //
CREATE PROCEDURE `sp_updateContest`(IN `p_id` INT, IN `p_game_id` INT, IN `p_contest_type_id` INT, IN `p_name` VARCHAR(50), IN `p_description` VARCHAR(50), IN `p_banner_url` VARCHAR(50), IN `p_begins_at` DATETIME, IN `p_ends_at` DATETIME)
BEGIN
IF (p_id = 0) THEN
